...
const create = require('./blueprints/create')
const update = require('./blueprints/update')
const {
  EntityNotFoundError
...
class SequelizeDbAdapter {
  ...
  findByIds (idList) {
    return this.model.findAll({
      where: {
        [this.service.settings.idField || 'id']: idList
      }
    })
  }
  ...
  insertPopulated (entity) {
    return create(this.model, entity)
  }
  ...
  updateByIdPopulated (_id, entity) {
    return update(this.model, _id, entity)
  }
  ...
  removeById (_id) {
    return this.findById(_id).then(entity => {
      if (!entity) {
        return Promise.reject(new EntityNotFoundError(_id))
      }
  ...
  createCursor (params, isCounting) {
    if (params) {
      ...
      if (isCounting) { return this.model.count(q) } else { return this.model.findAll(q) }
    }

    if (isCounting) { return this.model.count() } else { return this.model.findAll() }
  }
  ...
  transformSort (paramSort) {
    ...
    if (Array.isArray(sort)) {
      let sortObj = []
      sort.forEach(s => {
        if (s.startsWith('-')) { sortObj.push([s.slice(1), 'DESC']) } else { sortObj.push([s, 'ASC']) }
      })
      return sortObj
    }
